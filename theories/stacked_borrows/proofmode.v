...
From simuliris.stacked_borrows Require Export notation primitive_laws.
...
Lemma sim_value_result v_t v_s Φ :
  Φ (ValR v_t) (ValR v_s) -∗ #v_t ⪯ #v_s {{ Φ }}.
Proof. iIntros "H". iApply sim_expr_base. iExists (ValR v_t), (ValR v_s). eauto. Qed.
Lemma sim_place_result l_t t_t T_t l_s t_s T_s Φ :
  Φ (PlaceR l_t t_t T_t) (PlaceR l_s t_s T_s) -∗ Place l_t t_t T_t ⪯ Place l_s t_s T_s {{ Φ }}.
Proof. iIntros "H". iApply sim_expr_base. iExists (PlaceR _ _ _), (PlaceR _ _ _); eauto. Qed.

Lemma tac_sim_value v_t v_s Φ Δ :
  envs_entails Δ (|==> Φ (ValR v_t) (ValR v_s)) → envs_entails Δ (Val v_t ⪯ Val v_s {{ Φ }}).
Proof.
...
Lemma tac_sim_place l_t l_s t_t t_s T_t T_s Φ Δ :
  envs_entails Δ (|==> Φ (PlaceR l_t t_t T_t) (PlaceR l_s t_s T_s)) →
  envs_entails Δ (Place l_t t_t T_t ⪯ Place l_s t_s T_s {{ Φ }}).
Proof. rewrite envs_entails_eq => ->. iIntros "H". iApply sim_bupd. by iApply sim_place_result. Qed.
Lemma tac_sim_place_no_bupd l_t l_s t_t t_s T_t T_s Φ Δ :
  envs_entails Δ (Φ (PlaceR l_t t_t T_t) (PlaceR l_s t_s T_s)) →
  envs_entails Δ (Place l_t t_t T_t ⪯ Place l_s t_s T_s {{ Φ }}).
...
Ltac sim_result_head :=
  ...
  | |- envs_entails _ (sim _ (λ _ _, bupd _) (Place _) (Place _)) =>
      eapply tac_sim_place_no_bupd
  | |- envs_entails _ (sim _ (λ _ _, sim_expr _ _ _ _) (Place _) (Place _)) =>
      eapply tac_sim_place_no_bupd
  | |- envs_entails _ (sim _ (λ _ _, sim _ _ _ _) (Place _) (Place _)) =>
      eapply tac_sim_place_no_bupd
...
Ltac source_result_head :=
  ...
  | |- envs_entails _ (source_red (Place _) (λ _, bupd _)) =>
      eapply tac_source_red_base_no_bupd
  | |- envs_entails _ (source_red (Place _) (λ _, sim_expr _ _ _ _)) =>
      eapply tac_source_red_base_no_bupd
  | |- envs_entails _ (source_red (Place _) (λ _, sim _ _ _ _)) =>
      eapply tac_source_red_base_no_bupd
  | |- envs_entails _ (source_red (Place _) (λ _, source_red _ _)) =>
      eapply tac_source_red_base_no_bupd
  | |- envs_entails _ (source_red (Place _) _) =>
      eapply tac_source_red_base; try iIntros (_ _)
  end.
