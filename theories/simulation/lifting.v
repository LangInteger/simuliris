...
Section fix_sim.
  ...
  Proof.
    ...
    iFrame.
    destruct Hstep as [Hred Hdet]. destruct (Hred P_s σ_s) as (e_s' & σ_s' & efs & H).
    by specialize (Hdet _ _ _ _ _ H) as [-> [-> ->]].
  Qed.
  ...
  (** Primitive reduction *)
  Lemma sim_lift_prim_step_both e_t e_s Φ:
      ...
      ∀ e_t' efs_t σ_t',
        ⌜prim_step P_t e_t σ_t e_t' σ_t' efs_t⌝ ==∗
          ∃ e_s' efs_s σ_s', ⌜prim_step P_s e_s σ_s e_s' σ_s' efs_s⌝ ∗
            ...
            ([∗ list] e_t0;e_s0 ∈ efs_t;efs_s, e_t0 ⪯ e_s0 [{lift_post Ω}])) -∗
    e_t ⪯{Ω} e_s [{ Φ }].
  Proof.
    iIntros "Hsim".
  ...
  Lemma sim_lift_call Φ fn v_t v_s :
    ...
    (of_call fn v_t) ⪯{Ω} (of_call fn v_s) [{ Φ }].
  Proof.
    iIntros "Hom Hv".
