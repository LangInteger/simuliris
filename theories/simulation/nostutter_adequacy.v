...
Section meta_level_simulation.
  ...
  Proof.
    rewrite /msim gsim_expr_unfold; intros Hsat Hreach.
  ...
  Lemma msim_step (e_t e_t' e_s: expr Λ) (σ_t σ_t' σ_s: state Λ):
    ...
    ∃ e_s' σ_s', tc (prim_step P_s) (e_s, σ_s) (e_s', σ_s') ∧ msim e_t' σ_t' e_s' σ_s'.
  Proof.
    rewrite /msim gsim_expr_unfold; intros Hsat Hreach Hstep.
    eapply sat_mono with (Q:= (|==> _)%I) in Hsat; last first.
    { iIntros "[Hσ Hsim]". iMod ("Hsim" with "[$Hσ //]") as "Hsim". iExact "Hsim". }
    eapply sat_bupd in Hsat.
  ...
  Lemma msim_progress (e_t e_s: expr Λ) (σ_t σ_s: state Λ):
    ...
    ¬ stuck P_t e_t σ_t.
  Proof.
    rewrite /msim gsim_expr_unfold; intros Hsat Hreach Hstuck.
    eapply sat_mono with (Q:= (|==> _)%I) in Hsat; last first.
    { iIntros "[Hσ Hsim]". iMod ("Hsim" with "[$Hσ //]") as "Hsim". iExact "Hsim". }
    eapply sat_bupd in Hsat.
    eapply sat_or in Hsat as [Hsat|Hsat].
...
Section simulation_behaviorally_related.
  ...
  Proof.
    ...
    eapply msim_step in Hsim as (e_s' & σ_s' & Hsteps & Hsim'); [|by auto..].
  ...
  Proof.
    ...
    apply msim_val in Hsim' as (v_s & σ_s'' & Hsteps & Hsat); last (by eauto using not_reach_stuck_pres_rtc); 
      [ | by eauto | by eauto].
...
Section adequacy_statement.
  ...
  Proof.
    ...
    - split; [|split].
      ...
      + by unshelve by eapply msim_safety.
  Qed.
