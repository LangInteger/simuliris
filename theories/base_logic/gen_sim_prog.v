...
Class gen_progPreG (F C : Type) (Σ : gFunctors) `{Countable F} := {
  gen_prog_preG_inG :> ghost_mapG Σ F C;
}.

Class gen_progPreNameG (F C : Type) (Σ : gFunctors) (gen_prog_name : gname) `{Countable F} := GenProgPreNameG {
  gen_prog_preNameG :> gen_progPreG F C Σ
}.

Class gen_sim_progG (F C_t C_s : Type) (Σ : gFunctors) `{Countable F} := GenSimProgG {
  gen_prog_name_target : gname;
  gen_prog_name_source : gname;
  gen_prog_inG_source :> gen_progPreNameG F C_s Σ gen_prog_name_source;
  gen_prog_inG_target :> gen_progPreNameG F C_t Σ gen_prog_name_target;
}.
...
Section definitions.
  Context `{Countable F, gen_prog_name : gname, hG : !gen_progPreNameG F C Σ gen_prog_name }.
...
Section gen_prog.
  Context {F C} `{Countable L, gen_prog_name : gname, gen_progPreNameG F C Σ gen_prog_name}.
...
Lemma gen_prog_init_names `{Countable F, !gen_progPreG F C Σ} p :
  ⊢ |==> ∃ γp : gname,
    let hG := GenProgPreNameG F C Σ γp in
    gen_prog_interp p ∗ ([∗ map] f ↦ K ∈ p, f @ K).
Proof.
  iMod (ghost_map_alloc_empty (K:=F) (V:=C)) as (γp) "Hp".
  iExists γp.
  iAssert (gen_prog_interp (hG:=GenProgPreNameG _ _ _ γp _ _ _) ∅) with "[Hp]" as "Hinterp";
    first by iFrame "Hp".
...
Lemma gen_prog_init `{Countable F, !gen_progPreG F C_t Σ, !gen_progPreG F C_s Σ} P_t P_s :
  ⊢ |==> ∃ _ : gen_sim_progG F C_t C_s Σ,
