...
(** Moving a read of a mutable reference up across code that *may* use that ref. *)

(* Assuming x : &mut i32 *)
Definition prot_mutable_reorder_read_up_unopt : expr :=
    let: "c" := InitCall in
...
Definition prot_mutable_reorder_read_up_opt : expr :=
    let: "c" := InitCall in
    let: "x" := new_place sizeof_scalar "i" in
    retag_place "x" MutRef TyFrz sizeof_scalar FnEntry "c";;
    (* crucial point: the earlier retag has given us permission to read *)
    let: "v" := Copy *{sizeof_scalar} "x" in
    Call #[ScFnPtr "f"] #[] ;;
...
Lemma prot_mutable_reorder_read_up `{sborGS Σ} :
  ⊢ log_rel prot_mutable_reorder_read_up_opt prot_mutable_reorder_read_up_unopt.
Proof.
  log_rel.
...
Section closed.
  ...
  Lemma prot_mutable_reorder_read_up_ctx : ctx_ref prot_mutable_reorder_read_up_opt prot_mutable_reorder_read_up_unopt.
  Proof.
    set Σ := #[sborΣ].
    apply (log_rel_adequacy Σ)=>?.
    apply prot_mutable_reorder_read_up.
  Qed.
End closed.
